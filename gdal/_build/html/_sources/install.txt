...
Le projet GDAL ne produit pas de binaire régulier téléchargeable (éxécutable) pour 
chaque version. Cependant, des efforts sont réalisé pour produire des binaire prêt 
à l'emploi.
...
bibliothèques, commandes et gestion de Python et la documentation de GDAL en plus 
d'autres choses (dont le visualiseur OpenEV). Plus d'informations est disponible 
sur le site de `FWTools <http://fwtools.maptools.org/>`_
...
L'effort `ELGIS <http://elgis.argeo.org/>`_ fournie des RPMs de différentes applications SIG pour Enterpris 
Linux et ses dérivés (RHEL, CentOS, Scientific Linux).

OpenSuSE
********

Les RPM de GDAL pour OpenSuSE avec d'autres outils géospatial sont publier sur http://download.opensuse.org/repositories/Application:/Geo/
...
Un nouveau UbuntisGIS est disponible sur `launchpad d'UbuntuGIS <https://launchpad.net/~ubuntugis>`_ 
...
* Tamas Szekeres maintien un ensemble complet de paquetages binaires pour Win32 
  ...
  Ces paquets sont basés sur le développement actuel et les branches stables compilé 
  à partir du SVN journalier de GDAL. Les paquets SDK correspondant sont également 
  disponibles pour téléchargement à partir de cette endroit. Les paquets -devel 
  sont basé sur la version en développement (1.9.0dev au moment de la rédaction), 
  et les paquets -stable sont basé sur la dernière branche de version stable 
...
* Des binaires Windows via MinGW sont disponible ici http://map.hut.fi/files/Geoinformatica/win32/
...
* Des éxécuables minimalistes pour windows sont disponible ici : http://download.osgeo.org/gdal/win32/1.6/gdalwin32exe160.zip
* D'autres plugins seront ajouté au même endroit (comme Oracle/OCI) : http://download.osgeo.org/gdal/win32/
* Des binaires avec plus de fonctionnalités pour Windows, dont python, proj et la 
  gestion c# sont disponible comme part du paquetage `FWTools <http://fwtools.maptools.org/>`_ 
...
large ensemble de logiciel open source geospatial pour les environnements Win32 
